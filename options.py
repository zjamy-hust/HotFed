...
def args_parser():
    ...
    parser.add_argument('--mode', type=int, default=6,
                        help='[0,1,2,3,4,5,6,7,8]。0表示FedAvg，随机选择客户端； \
                                    ...
                                    4表示fedProx; \
                                    5表示以任务acc为参考标准选取最优client； \
                                    6表示采用global model+linear2产生attention mask，然后将attention mask加到image上，然后送入local model+linear 1进行预测；训练时，只采用预测label对linear 1-local resnet-linear 2进行更新，不\
                                        更新global model；在客户端进行测试时，直接采用原图进行预测，而不加mask；训练好的模型上传到服务器进行聚合，然后进行测试，聚合参数包括local resnet、linear 1、linear 2。\
                                    7表示')
